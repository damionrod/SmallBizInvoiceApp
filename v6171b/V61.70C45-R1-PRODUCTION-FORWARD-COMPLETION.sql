-- V61.70C45-R1 production forward completion from reconciled partial state
...
commit;
-- Restore authenticated execution for the complete owner/report surface after final narrowing.
grant execute on function public.v6170c45_supplier_credit(uuid) to authenticated;
grant execute on function public.v6170c45_void_supplier_refund(uuid,text) to authenticated;
grant execute on function public.v6170c45_match_supplier_refund(uuid,uuid) to authenticated;
grant execute on function public.v6170c45_undo_supplier_refund_match(uuid) to authenticated;
grant execute on function public.v6170c45_general_ledger(date,date,text,text) to authenticated;
