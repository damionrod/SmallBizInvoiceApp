...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  const {planSlug,returnUrl}=await req.json();const {data:plan}=await client.from('plans').select('*').eq('slug',planSlug).eq('is_public',true).single();if(!plan?.stripe_price_id)return out({error:'This plan does not have a Stripe Price ID yet. Add one in Super Admin → Subscription plans.'},400);
