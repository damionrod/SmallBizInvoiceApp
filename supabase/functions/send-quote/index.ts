...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const body=`Hi ${q.customer_name||'Customer'},\n\nPlease find attached quote ${q.quote_number}.\n\nAmount (ex GST): ${money(q.quoted_price_ex_gst)}\nGST: ${money(q.gst_amount)}\nTotal: ${money(q.total_incl_gst)}\nValid until: ${q.valid_until||'—'}\n\nKind regards,\n${trading}${s.phone?`\n${s.phone}`:''}${s.email?`\n${s.email}`:''}`;
    const payload:any={from:`${senderName} <${FROM_EMAIL}>`,to:[to],subject,html:`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#24313a">${body.split(/\r?\n/).map((line:string)=>line?esc(line):'&nbsp;').join('<br>')}</div>`};
