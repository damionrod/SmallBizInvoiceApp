...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const values={customerName:owned.customer_name||'Customer',invoiceNumber:owned.invoice_number||'Invoice',tradingName:trading,companyName:s.company||trading,total:money(owned.total),balanceDue:money(owned.balance_due??owned.total),dueDate:owned.due_date||'',phone:s.phone||'',email:s.email||''};
    ...
    const payload:any={from:`${senderName.replace(/[<>]/g,'')} <${FROM_EMAIL}>`,to:[to],subject,html:`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#24313a;white-space:normal">${body.split(/\r?\n/).map((line:string)=>line?esc(line):'&nbsp;').join('<br>')}</div>`};
