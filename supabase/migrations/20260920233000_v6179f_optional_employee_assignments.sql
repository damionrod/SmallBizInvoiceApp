-- V61.79F Schedule employee assignment resilience.
-- Employee allocation is optional. Invalid/stale employee IDs are ignored while
-- valid employees remain assignable in the same atomic save.
begin;
...
begin
  ...
  where a.schedule_id=v_schedule.id
    ...
    and not (a.employee_id=any(v_employee_ids));

  insert into public.job_schedule_assignments(
    business_id,schedule_id,employee_id,created_by,updated_by
  )
  select v_business,v_schedule.id,employee_id,(select auth.uid()),(select auth.uid())
  from unnest(v_employee_ids) employee_ids(employee_id)
