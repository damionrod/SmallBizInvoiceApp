...
-- V61.79E: editing a job with the same employee must preserve assignment progress.
insert into public.job_schedule_assignments(
  business_id,schedule_id,employee_id,assignment_status,planned_hours,actual_hours,created_by,updated_by
)
values(
  :'business_a'::uuid,current_setting('v6179a.schedule_id')::uuid,:'employee_a'::uuid,
  'confirmed',3.5,2.25,:'owner_a'::uuid,:'owner_a'::uuid
)
on conflict (schedule_id,employee_id) do update set
  assignment_status='confirmed',planned_hours=3.5,actual_hours=2.25;

select public.v6179_save_schedule_with_assignments(
  current_setting('v6179a.schedule_id')::uuid,
  jsonb_build_object(
    'business_id',:'business_a'::uuid,'customer_id',:'customer_a'::uuid,
    'job_costing_id',:'job_a'::uuid,'quote_id',:'quote_a'::uuid,'invoice_id',:'invoice_a'::uuid,
    'title','V6179E progress-preservation test','timezone','Pacific/Auckland','status','scheduled',
    'start_at',now()+interval '1 hour','end_at',now()+interval '2 hours'
  ),
  array[:'employee_a'::uuid]
);
select pg_temp.assert_true('atomic save preserves retained assignment progress',
  exists(
    select 1 from public.job_schedule_assignments
    where schedule_id=current_setting('v6179a.schedule_id')::uuid
      and employee_id=:'employee_a'::uuid
      and assignment_status='confirmed' and planned_hours=3.5 and actual_hours=2.25
  ));
