...
['included modules are submitted',()=>assert(admin.includes('p_included_modules:modules.length?modules:[\'invoice_manager\']'))],
['hidden invoice module inclusion is preserved',()=>assert(admin.includes('data-plan-existing-modules')&&admin.includes("existingModules.includes('invoice_payments')")&&admin.includes("!visibleModuleSlugs.includes('invoice_payments')"))],
['stale plan renders are ignored',()=>assert(admin.includes('adminPlanRenderSequence')&&admin.includes('renderSequence!==adminPlanRenderSequence'))],
